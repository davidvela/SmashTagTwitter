            ...
            if let startIndex = (indices?.firstObject as? NSNumber)?.integerValue {
                if let endIndex = (indices?.lastObject as? NSNumber)?.integerValue {
                    ...
                    if length > 0 {
                        ...
                        // initialiced the ranges with random values =S
                                        nsrange = inText.rangeOfString("Hola", nearRange: NSMakeRange(startIndex, endIndex-startIndex))
                                         range = advance(inText.startIndex, 1)...advance(inText.startIndex, 2)
